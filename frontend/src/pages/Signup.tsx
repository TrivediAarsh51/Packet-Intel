...
const Signup: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.detail || 'Failed to register investigator profile. Please check details.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 text-slate-100 relative overflow-hidden px-4">
      ...
      <div className="w-full max-w-md glass-card p-8 border border-slate-800 shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Access Forensic Sniffer & DPI Tools</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                required
                className="w-full bg-slate-900/60 border border-slate-800 rounded-lg pl-11 pr-4 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all duration-200"
                placeholder="investigator_id"
                value={username}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                required
                className="w-full bg-slate-900/60 border border-slate-800 rounded-lg pl-11 pr-11 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all duration-200"
                placeholder="••••••••"
                value={password}
          ...
          <button
            type="submit"
            className="w-full py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-medium rounded-lg transition-all duration-200 shadow-lg shadow-blue-500/20 active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none text-sm font-semibold mt-6"
            disabled={loading}
          >
